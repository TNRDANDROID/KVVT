...
import com.nic.KVVTSurvey.utils.FontCache;
...
public class LoginScreen extends AppCompatActivity implements View.OnClickListener, Api.ServerResponseListener {
    ...
    public LoginScreenNewBinding loginScreenBinding;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        loginScreenBinding = DataBindingUtil.setContentView(this, R.layout.login_screen_new);
...
//        loginScreenBinding.password.setText("test123#$");
    ...
    public JSONObject schemeListJsonParams() throws JSONException {
        String authKey = Utils.encrypt(prefManager.getUserPassKey(), getResources().getString(R.string.init_vector), Utils.schemeListDistrictBlockWiseJsonParams(this).toString());
        JSONObject dataSet = new JSONObject();
        dataSet.put(AppConstant.KEY_USER_NAME, prefManager.getUserName());
        dataSet.put(AppConstant.DATA_CONTENT, authKey);
    ...
    public class InsertVillageTask extends AsyncTask<JSONObject, Void, Void> {
        ...
        protected Void doInBackground(JSONObject... params) {
            dbData.open();
            ArrayList<KVVTSurvey> villagelist_count = dbData.getAll_Village(prefManager.getDistrictCode(),prefManager.getBlockCode());
    ...
    public class InsertHabTask extends AsyncTask<JSONObject, Void, Void> {
        ...
        protected Void doInBackground(JSONObject... params) {
            dbData.open();
            ArrayList<KVVTSurvey> hablist_count = dbData.getAll_Habitation(prefManager.getDistrictCode(),prefManager.getBlockCode());
            if (hablist_count.size() <= 0) {
                if (params.length > 0) {
                    ...
                    for (int i = 0; i < jsonArray.length(); i++) {
                        KVVTSurvey habListValue = new KVVTSurvey();
                        try {
